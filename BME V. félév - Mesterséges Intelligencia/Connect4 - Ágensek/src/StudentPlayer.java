...
public class StudentPlayer extends Player{
    ...
    public int step(Board board) {
	    ...
	    for (int lepes : board.getValidSteps()) {
			...
			int most=MinMaxDontes(uj,7,-999,999);
			if(most>maxertek) {
				maxertek=most;
				mitlepjunk = lepes;
			}
    ...
    private int MinMaxDontes(Board board, int melyseg, int alpha, int beta) {
		return MinErtek(board, melyseg, alpha, beta);
		
	}


	int MaxErtek(Board allapot, int melyseg,  int alpha, int beta ) {
    	if(VegeTeszt(allapot) || melyseg == 0) {
    		return Hasznossag(allapot);
    	}
    	int k = -100;
    	for(Board s: KovetoAllapotok(allapot)) {
    		k = Math.max(k, MinErtek(s,melyseg-1,alpha,beta));
    		if(k >= beta) return k;
    ...
    int MinErtek(Board allapot, int melyseg, int alpha, int beta) {
    	if(VegeTeszt(allapot) || melyseg == 0) {
    		return Hasznossag(allapot);
    	}
    	...
    	int v = 100;
    	for(Board s: KovetoAllapotok(allapot)) {
    		v = Math.min(v, MaxErtek(s,melyseg-1,alpha,beta));
    		if(v <= alpha) return v;
