...
public class Game implements Description {
    ...
    public boolean betweenRounds(boolean gameOver){
    ...
    public boolean incrementRound(boolean gameOver) {
        ...
        for(Cleaner cleaner : cleaners) {
            int i;
            if(cleaner.getRoom().getNeighbours().size()>0){
                i = random.nextInt(cleaner.getRoom().getNeighbours().size());
                cleaner.goToRoom(cleaner.getRoom().getNeighbours().get(i));
            }
...
/**
     * A játékban lévő tanárokat adja vissza
     * @return játékban lévő tanárok listája
     */
